package com.sz.being.dao;

import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;

import com.sz.being.model.Diary;
...
public class DBManager {
    ...
    public void add(Diary diary) {
        ...
        try {
            db.execSQL("insert into being(diary_label,diary_content,diary_mood,diary_weather,diary_type) values(?,?,?,?,?)",
    ...
    public void updateDiary(Diary diary) {
        db.execSQL("update being set diary_label = ? , diary_content = ?,diary_mood = ?,diary_weather = ? where _id = ? ",
    ...
    public void deleteDiary(String id) {
        db.delete("being", "_id = ?", new String[]{id});
    }

    /**
     * delete being to draft
     */
    ...
    public void movetotrash(String id) {
        db.execSQL("update being set diary_type = ? where _id = ? ", new String[]{"trash", id});
    }
    ...
    public Cursor queryTheCursor() {
        c = db.rawQuery("SELECT * FROM being  where diary_type = ? ORDER BY diary_date DESC", new String[]{"useful"});
        return c;
    }
    ...
    public Cursor queryTrash() {
        c = db.rawQuery("SELECT * FROM being  where diary_type = ? ORDER BY diary_date DESC", new String[]{"trash"});
        return c;
    }
    ...
    public Cursor find(String key) {
        c = db.rawQuery("SELECT * FROM being where diary_content like ? or diary_label like ? or " +
    ...
    public Cursor findTrash(String key) {
        c = db.rawQuery("SELECT * FROM being where diary_type = ? and (diary_content like ? or diary_label like ? or " +
    ...
    public Cursor queryId(String id) {
        c = db.rawQuery("SELECT * FROM being where _id = ? ", new String[]{id});
        return c;
    }
    ...
    public void deleteAllDiary() {
        db.delete("being", "diary_type = ?", new String[]{"trash"});
    }

    public void recoverDiary(String id1) {
        db.execSQL("update being set diary_type = ? where _id = ?", new String[]{"useful", id1});
    }
}
